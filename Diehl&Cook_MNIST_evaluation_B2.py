...
Updated by Genevieve Fahey to use Brian 2 and Python 3.x instead of Brian 1 and Python 2.x
'''
...
def get_new_assignments(result_monitor, input_numbers):
    ...
    for j in range(10):
        ...
        print('num_inputs', num_inputs, 'for label', j)
        if num_inputs > 0:
...
r = end_time_testing # added for ease of use, replaces instances of '10000' in code, usually set to end_time_testing
...
print('load results')
training_result_monitor = np.load(data_path + 'resultPopVecs' + training_ending + '.npy')
...
print(assignments)
counter = 0
num_tests = end_time_testing / r
sum_accurracy = [0] * int(num_tests)
while (counter < num_tests):
    end_time = min(end_time_testing, r*(counter+1))
    start_time = r*counter
